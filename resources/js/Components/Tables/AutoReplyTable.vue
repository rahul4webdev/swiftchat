<script setup>
    ...
    const deleteAction = (key) => {
        form.delete('/canned-replies/' + key);
    }
...
<template>
    <div class="md:bg-white flex items-center border border-primary md:border-none md:shadow-sm h-10 w-full md:w-80 rounded-[0.5rem] mb-6 text-sm">
        <span class="pl-3">
        ...
        <input @input="search" v-model="params.search" type="text" class="outline-none px-4 w-full" :placeholder="$t('Search by name or trigger text')">
            ...
            <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                ...
                <TableBodyRowItem :position="'last'">
                    <Dropdown :align="'right'" class="mt-2">
                        ...
                        <template #items>
                            <DropdownItemGroup>
                                <DropdownItem :href="'/canned-replies/' + item.uuid + '/edit'">{{ $t('Edit') }}</DropdownItem>
